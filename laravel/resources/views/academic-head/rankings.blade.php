...
<div style="max-width:1040px;">
    ...
    <form method="GET" style="display:flex;gap:10px;margin-bottom:16px;">
        ...
        <input name="search" value="{{ $search }}" placeholder="Search by name or roll…"
            ...
            onfocus="this.style.borderColor='#7a95c8'" onblur="this.style.borderColor='rgba(245,241,232,0.1)'">
        ...
        <button type="submit" style="background:#7a95c8;border:none;border-radius:8px;padding:9px 18px;color:#08080a;font-size:13px;font-weight:700;cursor:pointer;">Filter</button>
        @if($search || $batchFilter)
        <a href="{{ route('academic-head.rankings', ['test_id'=>$testId]) }}" style="background:transparent;border:1px solid rgba(245,241,232,0.08);border-radius:8px;padding:9px 14px;color:#6a665f;font-size:13px;text-decoration:none;display:flex;align-items:center;">Clear</a>
        @endif
    </form>
            ...
            <tbody>
                ...
                <tr style="border-bottom:1px solid rgba(245,241,232,0.04);"
                    ...
                    <td style="text-align:center;padding:12px 10px;">
                        <span style="font-size:10px;color:#7a95c8;background:rgba(122,149,200,0.1);border-radius:4px;padding:2px 7px;">{{ $r->batch_name }}</span>
                    </td>
                    ...
                    <td style="text-align:right;padding:12px 16px;">
                        <a href="{{ route('academic-head.student.deep-dive', $r->student_id) }}"
                           style="font-size:11px;font-weight:600;color:#7a95c8;text-decoration:none;background:rgba(122,149,200,0.1);border-radius:5px;padding:5px 10px;border:1px solid rgba(122,149,200,0.2);">
                            View →
                        </a>
                    </td>
                </tr>
