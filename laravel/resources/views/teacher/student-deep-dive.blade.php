...
.tab-btn.active { color:#7fb685;border-bottom-color:#7fb685; }
...
<div style="max-width:980px;">
    ...
    <div style="margin-bottom:18px;">
        <a href="{{ route('teacher.students') }}" style="font-size:12px;color:#6a665f;text-decoration:none;">← My Students</a>
    </div>
    ...
    <div style="background:#14141b;border:1px solid rgba(245,241,232,0.08);border-radius:12px;padding:24px;margin-bottom:16px;display:flex;align-items:center;gap:20px;">
        <div style="width:52px;height:52px;border-radius:50%;background:rgba(127,182,133,0.15);display:flex;align-items:center;justify-content:center;font-size:20px;font-weight:700;color:#7fb685;flex-shrink:0;">
        ...
        <div style="flex:1;min-width:0;">
            ...
            <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
                ...
                @if($subject)<span style="font-size:12px;color:#6a665f;">{{ $subject->name }}</span>@endif
            </div>
        </div>
        {{-- Quick KPIs --}}
        ...
        <div style="display:flex;gap:24px;flex-shrink:0;">
            @foreach([
                ...
                ['Best Rank',   $bestRank ? '#'.$bestRank : '—', '#7fb685'],
    ...
    <div id="tab-overview" class="tab-pane active">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
            ...
            {{-- Top Topics --}}
            <div style="background:#14141b;border:1px solid rgba(245,241,232,0.08);border-radius:10px;padding:20px;">
                <p style="font-size:11px;font-weight:600;color:#a8a39c;letter-spacing:1px;text-transform:uppercase;margin:0 0 14px;">Top Topics · {{ $subject?->name }}</p>
                @forelse($topicMastery->sortByDesc('mastery_percentage')->take(5) as $t)
                @php $mp = (float)$t->mastery_percentage; $tc = $mp >= 70 ? '#7fb685' : ($mp >= 40 ? '#d4a574' : '#c87064'); @endphp
                <div style="margin-bottom:10px;">
                    <div style="display:flex;justify-content:space-between;margin-bottom:3px;">
                        <span style="font-size:11px;color:#a8a39c;">{{ $t->topic_code }} · {{ substr($t->topic_name,0,28) }}</span>
                        <span style="font-size:11px;font-weight:700;color:{{ $tc }};">{{ round($mp) }}%</span>
                    </div>
    ...
    <div id="tab-mastery" class="tab-pane">
        ...
        <div style="background:#14141b;border:1px solid rgba(245,241,232,0.08);border-radius:10px;padding:20px;margin-bottom:14px;">
            <p style="font-size:12px;font-weight:700;color:#7fb685;text-transform:uppercase;letter-spacing:0.8px;margin:0 0 16px;">{{ $subjectName }}</p>
        ...
        <div style="background:#14141b;border:1px solid rgba(245,241,232,0.08);border-radius:10px;padding:20px;">
            ...
            <div style="display:flex;align-items:center;gap:14px;padding:12px 0;border-bottom:1px solid rgba(245,241,232,0.04);">
                ...
                <div style="flex:1;min-width:0;">
                    ...
                    <p style="font-size:10px;color:#6a665f;margin:0;">{{ $t->topic_code }} · {{ $t->total_questions_correct ?? 0 }}/{{ $t->total_questions_attempted ?? 0 }} correct</p>
                </div>
            ...
            <table style="width:100%;border-collapse:collapse;">
                ...
                <tbody>
                    ...
                    <tr style="border-bottom:1px solid rgba(245,241,232,0.04);"
                        ...
                        <td style="text-align:center;padding:11px 10px;font-size:12px;color:#7a95c8;">{{ round($t->percentile,1) }}%</td>
